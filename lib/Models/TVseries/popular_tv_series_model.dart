...
class PopularTVseriesModelApi {
  Future<List<PopularTVseriesModel>> getPopularTVseries() async {
    const url = '${ConstantKeys.popularTVseriesUrl}api_key=${ConstantKeys
        .apiKey}';

    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body)['results'] as List;
      return data.map((popularTVseries) =>
          PopularTVseriesModel.fromJson(popularTVseries)).toList();
    } else {
